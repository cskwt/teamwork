...
const ProjectsPage: React.FC<ProjectsPageProps> = ({ onOpenBoard }) => {
  ...
  const userDeptIds = currentUser?.departmentIds?.length
    ...
    <div className={`projects-page ${isPhone ? 'projects-page--phone' : ''}`}>
      ...
      <div className="dept-cards-container">
        {[...visibleDepts].reverse().map((dept) => {
          ...
          const allDeptOrders = orders.filter((o) => !o.deletedAt && !o.archivedAt && !o.purgedAt && !o.isOrderRequest && !o.digitalPrinting && !o.largeFormat && o.departmentId === dept.id && o.status !== 'cancelled');
